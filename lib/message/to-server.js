...
module.exports = {
  login: (username, password) => {
    ...
    let msg = new Message().int32(1).str(username).str(password)
    msg.int32(VERSION).str(hash).int32(MINOR)
    return msg
  },
  setWaitPort: (port) => new Message().int32(2).int32(port),
  ...
  branchLevel: (level) => new Message().int32(126).int32(level),
  branchRoot: (root) => new Message().int32(127).str(root),
  childDepth: (depth) => new Message().int32(129).int32(depth),
  ...
  messageUsers: (usernames, message) => {
    let msg = new Message().int32(149).int32(usernames.length)
