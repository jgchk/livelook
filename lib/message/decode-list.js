...
function decodeShares(packet, compressed = true) {
  let dirs = {}

  let unzipped

  if (compressed) {
    unzipped = packet.data.slice(packet.pointer, packet.data.length)
    // TODO we have to do this with async
  ...
  for (let i = 0; i < dirCount; i += 1) {
    let dirName = unzipped.str().replace(/\\/g, '/')
...
export const files = decodeFiles
export const shares = decodeShares
