...
import TransferPeer from '../transfer-peer'

export default (livelook, client) => {
  const handlers = {
    ...
    connectToPeer: (res) => {
      switch (res.cType) {
        ...
        case 'D': {
          ...
          let distribPeer = new DistribPeer(res)
          distribPeer.attachHandlers(livelook)
          distribPeer.init(() => distribPeer.pierceFirewall())
      ...
      // TODO lets genericize this a bit more
    },
    messageUser: (res) => {
    ...
    roomTickerAdd: (res) => {
      try {
        ...
        let err = new Error(
          'server sent us a ticker to invalid ' +
    ...
    roomTickerRemove: (res) => {
      try {
        ...
        let err = new Error(
          'server told us to delete ticker to ' +
  ...
  for (const h of Object.keys(handlers)) client.on(h, handlers[h])
}
